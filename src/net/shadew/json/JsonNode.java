...
public interface JsonNode extends Iterable<JsonNode>, JsonRepresentable {
    ...
    static JsonNode fromJavaObject(Object obj) {
        if (obj == null) {
        ...
        } else if (obj instanceof Stream<?> itr) {
            // Stream -> Array
            ArrayNode node = new ArrayNode();
            itr.map(JsonNode::fromJavaObject).forEach(node::add);
            return node;
        } else if (obj instanceof Iterable<?> itr) {
            // Iterable -> Array
            ArrayNode node = new ArrayNode();
            for (Object el : itr)
                node.add(fromJavaObject(el));
            return node;
        } else if (obj instanceof Iterator<?> itr) {
            // Iterator -> Array
            ArrayNode node = new ArrayNode();
            while (itr.hasNext())
                node.add(fromJavaObject(itr.next()));
            return node;
        } else if (obj instanceof Enumeration<?> itr) {
            // Enumeration -> Array
            ArrayNode node = new ArrayNode();
            ...
            return node;
        } else if (obj instanceof Dictionary<?, ?> map) {
            // Dictionary -> Object
            ObjectNode node = new ObjectNode();
